...
1. Prisma Postgres 또는 일반 Postgres
   - Vercel Marketplace에서 Prisma Postgres를 추가하거나 외부 Postgres를 연결합니다.
   - `DATABASE_URL` 환경변수가 프로젝트에 연결되어야 합니다.
   - 앱이 처음 저장 API를 호출할 때 `serve_reports` 테이블을 자동 생성합니다.

2. Vercel Blob
   - Vercel Storage에서 Blob Store를 추가합니다.
   - `BLOB_READ_WRITE_TOKEN` 환경변수가 프로젝트에 연결되면 캡쳐 이미지를 Blob public URL로 저장합니다.
   - 토큰이 없으면 캡쳐 이미지는 DB의 리포트 JSON 안에 저장됩니다. 초기 테스트는 가능하지만 장기 운영에는 Blob 연결을 권장합니다.
...
- `공유하기`를 누르면 리포트 텍스트, 서브 종류, 캡쳐 정보가 Postgres에 저장됩니다.
- `BLOB_READ_WRITE_TOKEN`이 있으면 캡쳐 이미지는 Vercel Blob에 저장되고, DB에는 이미지 URL만 저장됩니다.
